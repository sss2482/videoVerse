...
def user_registration(l):
	...
	db=MySQLdb.connect("localhost",username,password,"yt_db")
	c=db.cursor()
	query="select count(*) from users where email= '%s' " %(l[4])
	print(query)
	...
	if result[0][0]:
		return 0
	c.execute("insert into users values (%s,%s,%s,%s,%s,%s)", (l[0],l[1],l[2],str(l[3]),l[4],l[5]))
	c.execute("commit")
...
def non_signed_history(video_id):
	...
	db=MySQLdb.connect("localhost",username,password,"yt_db")
	c=db.cursor()
	query ="select * from history where history.video_id= '%s'" % (video_id)
	status=c.execute(query)
	if status:
		query="select view_count from history where history.video_id= '%s'" % (video_id)
		view_count=c.execute(query)
		data = c.fetchall ()
		#print( data[0][0]
		# print( view_count
		...
		#print( view_count
		query="update history set view_count= %d" %view_count +" where video_id= '"+video_id+"'"
		#print( query
		# query="update into history where history.video_id= '%s' set history.view_count= %d " %(video_id,view_count)
	else:
		query="insert into history values('%s',%d)"%(video_id,1)
	result=c.execute(query)
	c.execute('commit')
	return 1
	#c.execute("insert into history values (%s,%s)");
	#c.execute("commit");

def history(user):
	db=MySQLdb.connect("localhost",username,password,"mysql")
	c=db.cursor()
	db=MySQLdb.connect("localhost",username,password,user.id)
	c=db.cursor()
	table_name=user+'_history'
